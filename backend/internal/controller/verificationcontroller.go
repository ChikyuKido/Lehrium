...
func SendNewVerificationEmail(c *gin.Context) {
...
func sendFirstVerificationEmail(uuid string, email string) {
	from := os.Getenv("GMAIL_USERNAME")
	pass := os.Getenv("GMAIL_APPPASSWORD")
	to := email
	...
	msg := "From: Lehrium Verification" + "\n" +
		"To: " + to + "\n" +
		"Subject: Lehrium Account verification\n\n" +
		"please verify your account via this link(expires in 5 minutes): \n" +
		"https://localhost:8080/auth/verifyEmail?uuid=" + uuid
	...
	updateVerificationEmailDate(uuid)
	log.Println("Successfully sended to " + to)
}
